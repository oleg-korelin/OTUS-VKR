...
Процедура ТоварыЦенаПриИзменении(Элемент)
	Стр = ПолучитьТекущуюСтрокуТовары();
	Стр.Сумма = Стр.Количество * Стр.Цена - Стр.СуммаСертификата;
	ЗаполнитьДополнительныеДанныеСтроки(Стр);
КонецПроцедуры
...
&НаСервереБезКонтекста
Процедура ПолучитьОстатокНаСервере(ПодарочныйСертификат)
	ОтправкаСообщенийRMQ.ПроверитьПодключениеСервер();
	ТекстСообщения = ПодарочныйСертификат;
	ТочкаОбмена = "references";
	ИмяОчереди = "reference2";
	КлючМаршрутизации = "reference2";
...
Процедура ПолучитьОстаток(Команда)
	ПолучитьОстатокНаСервере(Объект.ПодарочныйСертификат);
	Объект.РезультатПолучен = Ложь;
	Объект.СуммаПодарочногоСертификата = 0;
КонецПроцедуры

&НаСервере
Процедура ПроверитьПолучениеОстаткаНаСервере()
	
	КлиентКомпоненты = ПолучениеСообщенийRMQ.ПолучитьКомпонентуСервер();
	
	Результат = ПолучениеСообщенийRMQ.ПрочитатьСообщениеКлиентСерверФунк(КлиентКомпоненты);
	Результат = СтрЗаменить(Результат, " ", "");
	Результат = СтрЗаменить(Результат, Символы.НПП, "");
	Если Результат <> "" Тогда 
		Объект.СуммаПодарочногоСертификата = Число(Результат);
		Объект.РезультатПолучен = Истина;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПроверитьПолучениеОстатка(Команда)
	ПроверитьПолучениеОстаткаНаСервере();
КонецПроцедуры

&НаКлиенте
Процедура СуммаСписанияСертификатаОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	
	Если  Число(Текст) > Объект.СуммаПодарочногоСертификата Тогда
		 Объект.СуммаСписанияСертификата = Строка(Объект.СуммаПодарочногоСертификата);
	КонецЕсли;
...
Процедура Применить(Команда)
	Если Объект.СуммаСписанияСертификата > 0 Тогда
	Стр = ПолучитьТекущуюСтрокуТовары();
	Стр.СуммаСертификата = Объект.СуммаСписанияСертификата;
	Стр.Сумма = Стр.Количество * Стр.Цена - Стр.СуммаСертификата;
	ЗаполнитьДополнительныеДанныеСтроки(Стр);	
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ТоварыСуммаСертификатаПриИзменении(Элемент)
	Стр = ПолучитьТекущуюСтрокуТовары();
	Стр.Сумма = Стр.Количество * Стр.Цена - Стр.СуммаСертификата;
	ЗаполнитьДополнительныеДанныеСтроки(Стр);
КонецПроцедуры
