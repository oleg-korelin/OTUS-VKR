...
Процедура ПриПроведенииРегистрацииПодарочныхСертификатовОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
...
Процедура ПриПроведенииРасходаТовараОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
		
	Если Источник.ОбменДанными.Загрузка Тогда 
		Возврат;
	КонецЕсли;
	
	Если Источник.СуммаСписанияСертификата = 0 Тогда 
		Возврат;
	КонецЕсли;
	
	СтруктураРасхода = Новый Структура;
	СтруктураРасхода.Вставить("sertifikat", Источник.ПодарочныйСертификат);
	СтруктураРасхода.Вставить("summ_spisaniya", Источник.СуммаСписанияСертификата);
	
	Запись = Новый ЗаписьJSON;
	Запись.УстановитьСтроку();
	ЗаписатьJSON(Запись, СтруктураРасхода);
	ТекстСообщения = Запись.Закрыть();
	СправочникОбъект = Справочники .ИсходящиеСообщенияRMQ.СоздатьЭлемент();
	СправочникОбъект.Сообщение = ТекстСообщения;
	СправочникОбъект.ИмяОчереди = "reference4";
	СправочникОбъект.ТочкаОбмена  = "references";
	СправочникОбъект.КлючМаршрутизации = "reference4";
	СправочникОбъект.КОтправке = Истина;
	СправочникОбъект.Записать();
